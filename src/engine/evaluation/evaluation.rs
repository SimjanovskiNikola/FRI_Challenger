...
impl EvaluationTrait for Board {
    ...
    fn evaluation(&mut self) -> isize {
        ...
        println!("Imbalance White: {:?}", self.imbalance(WHITE));
        println!("Imbalance Black: {:?}", self.imbalance(BLACK));
        println!("Imbalance Total: {:?}", (self.imbalance(WHITE) - self.imbalance(BLACK)) / 16);
    ...
    fn get_imbalance_pce_cnt(&self, num: usize, clr: Color) -> isize {
        match num {
            0 => self.king_bb(clr).count() as isize,
